...
class DB
{
   ...
   private function __construct($instancia = 1)
   {
      try {
         ...
         if ($instancia == 1) {
            ...
            /* lo que estaba al dia 10/03/2021
            $DB_HOST="131.110.1.27\TESTMSSQL";
            $DB_NAME="IMPPRUEBAS";
            $DB_USER="GscSqlPqr";
            $DB_PASSWORD="2G8S1P4rT";
            */
            $DB_HOST="131.110.1.21";
            $DB_NAME="Impuestos";
            $DB_USER="GscSqlPqr";
            $DB_PASSWORD="2G8S1P4rT";
            $this->_pdo = new PDO('sqlsrv:Server=' . $DB_HOST . ';Database=' . $DB_NAME, $DB_USER, $DB_PASSWORD);
...
//            H::dnd($this->_pdo);
   ...
   {
      //var_dump($this->_pdo->prepare($sql));
      if ($this->_query = $this->_pdo->prepare($sql)) {
         ...
         if ($this->_query->execute()) {
            if ($class) {
               ...
               if (preg_match('/update/i', $sql) || preg_match('/insert into/i', $sql) || preg_match('/delete/i', $sql)) {
   ...
   {
      return $this->query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE	TABLE_NAME = '{$table}'")->results();
