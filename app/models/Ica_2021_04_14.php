...
class Ica extends Model
{
   ...
   public static function validarIca($Nit)
   {
      ...
      $sql1 = "SELECT [MaeNum] FROM [MAEIC] WHERE [MaeProCod] = '{$Nit}'";
   ...
   public static function validarRespuestas($nit, $razon_social, $direccion, $telefono)
   {
      ...
      $sql = "SELECT RetINit
      FROM [RETINDC]
      WHERE replace([RetINit], ' ', '')='{$nit}' and RetIRazS like '%{$razon_social}' and [RetIDir] like '%{$direccion}' and RetITelPri like '%{$telefono}';";
      //H::dnd($sql);
      if ($db->query($sql)->count() > 0) {
         return $db->query($sql, [])->first();
      } else
   ...
   public static function updateAgente($model)
   {
      ...
      BEGIN
         ...
         BEGIN
            ...
            ELSE
            BEGIN
            INSERT INTO [RETINDCL] ([RetINit],[RetNitRepL],[RetINomRep],[RetITipIde],[RetITelRep],[RetIPNomRep] ,[RetISNomRep] ,[RetIPApeRep] ,[RetISApeRep])
            VALUES('{$model->nit}','{$model->nit_representante}','{$nombre_repre}','{$model->tipo_documento_representante}','{$model->telefono_representante}',
            '{$model->nombre_representante}' ,'{$model->nombre2_representante}' ,'{$model->apellido_representante}','{$model->apellido2_representante}');
            END
         END
